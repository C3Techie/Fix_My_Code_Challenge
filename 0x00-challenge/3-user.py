...
class User:
    ...
    def password(self, new_password):
        """
        Password setter:
        - `None` if `new_password` is `None`
        - `None` if `new_password` is not a string
        - Hash `new_password` in MD5 before assigning to `__password`
        """
    ...
    def is_valid_password(self, password):
        ...
        - `False` if `password` is not a string
        - `False` if `__password` is `None`
        - Compare `__password` and the MD5 value of `password`
        """
        if password is None or not isinstance(password, str):
            return False
        if self.__password is None:
            return False
        return hashlib.md5(password.encode()).hexdigest().lower() == self.__password
